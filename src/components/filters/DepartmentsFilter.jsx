...
const DepartmentsFilter = forwardRef(({ open, setOpen, data }, ref) => {
  ...
  const handleFilter = () => {
    ...
    selectedOptions.forEach((option) => {
      const exists = updatedFilters.some(
        (filter) => filter.type === "department" && filter.value === option
      );

      if (!exists) {
        updatedFilters.push({ type: "department", value: option });
      }
    });
...
DepartmentsFilter.displayName = "DepartmentsFilter";
